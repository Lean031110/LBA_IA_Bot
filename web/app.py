...
def create_app():
    ...
    def load_user(user_id):
        try:
            with sqlite3.connect(Config_Web.SQLALCHEMY_DATABASE_URI.split('///')[-1]) as conn:
                cursor = conn.cursor()
    ...
    def admin_required(f):
        ...
        def decorated_function(*args, **kwargs):
            if not current_user.is_authenticated or current_user.id != Config_Web.CREATOR_ID:
        ...
        if request.method == 'POST':
            ...
            try:
                with sqlite3.connect(Config_Web.SQLALCHEMY_DATABASE_URI.split('///')[-1]) as conn:
                    ...
                    if user and user[0] == Config_Web.CREATOR_ID:
